...
namespace Health_Tracker.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SetBedtime(string bedtime)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null) return NotFound();

            if (TimeSpan.TryParse(bedtime, out var parsedBedtime))
            {
                user.BedTime = parsedBedtime;
                await _userManager.UpdateAsync(user);
                TempData["Success"] = "Bedtime set successfully! You'll receive a reminder email at " + bedtime;
            }
            else
            {
                TempData["Error"] = "Invalid bedtime format. Please try again.";
            }
